...
def save_photo(photo_url, save_dir, timeout=10):
    ...
    with request.urlopen(photo_url, timeout=timeout) as res, save_path.open('wb') as f:
        ...
        print('已下载图片:{dir_name}/{photo_name},请求的URL为:{url}'
              .format(dir_name=save_dir, photo_name=photo_name,url=photo_url))
...
if __name__ =='__main__':
    ...
    request_headers = {
        'Refere': 'http://www.toutiao.com/search/?keyword=%E7%BE%8E%E5%A5%B3',
    ...
    while ongoing:
        ...
        for a_url in article_urls:
            ...
            try:
                ...
                doenload_dir = _create_dir(root_dir/dir_name)

                # 开始下载文章中的图片
                for p_url in photo_urls:
                    try:
                        save_photo(p_url, save_dir=doenload_dir)
